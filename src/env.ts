import { z } from 'zod'
import { besideDatabase } from './lib/data-dir.js'
...
const EnvSchema = z
  .object({
    ...
    // Art cache (brief, 3.10). Sem padrão PRÓPRIO: ausente, ele deriva do banco
    // (`lib/data-dir.ts`, abaixo), que é o único caminho que o Docker e o Electron
    // declaram. O padrão antigo, `./data/art`, era relativo ao cwd e caía fora do
    // volume `/data` — a arte se perdia em toda recriação do container.
    WATCHPILE_ART_CACHE_PATH: z.string().min(1).optional(),
